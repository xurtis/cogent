      ...
      ((nth_arg_conv_tac 7 (* \<gamma> *) 1 ctxt (Simplifier.rewrite ctxt)
       ...
       ((rule corres_var 1
        ...
        (rule corres_afun 1
         ...
         THEN subgoal_val_rel_simp_add [] 1) (* TODO maybe add \<open>Option.option.sel\<close> simps *)
...
fun generate_HO_absfun_corres (xi:term) ctxt (fname:string) (callees:(term * string) list) min_measure = let
  ...
  val prop = if isAutoCorresFunRec ctxt fname
    then @{mk_term "\<lbrakk> i < length \<gamma>; val_rel (\<gamma> ! i) v'; \<Gamma> ! i = Some (fst (snd (the (?Xi ?fname)))); m \<ge> ?min_m \<rbrakk> \<Longrightarrow>
    ...
    else @{mk_term "\<lbrakk> i < length \<gamma>; val_rel (\<gamma> ! i) v'; \<Gamma> ! i = Some (fst (snd (the (?Xi ?fname)))) \<rbrakk> \<Longrightarrow>
                    ?corres ?state_rel (App (AFun ?fname []) (Var i))
